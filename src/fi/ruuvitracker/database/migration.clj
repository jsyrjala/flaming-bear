(ns fi.ruuvitracker.database.migration
...
(defprotocol DatabaseMigrator
  (migrate-forward [this])
  (migrate-backward [this])
  (list-migrations [this]))

(trapperkeeper/defservice database-migrator
  "doc"
  DatabaseMigrator
  ...
  (migrate-backward [this]
                    (execute-rollback (get-in-config [:database :db-spec])))

  (list-migrations [this]
                   (execute-list (get-in-config [:database :db-spec])))
  )
